 ...
 * a bit different in several respects: (1) it supports dragging to a AbsList view
 * and trash area; (2) it dynamically adds drop targets when a drag-drop
 * sequence begins. The child views of the AbsListView are assumed to implement the
...
public class DragLayer extends FrameLayout implements DragController.DragListener {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        scrollList(DragController.clamp((int)ev.getRawY(), 0, mDragController.getDisplayMetrics().heightPixels));
    ...
    /**
     * Get the value of the AbsListView property.
     * 
     * @return AbsListView
     */
    ...
    /**
     * Set the value of the AbsListView property.
     * 
     * @param newValue AbsListView
     */

    public void setAbsListView(AbsListView newValue) {
        mAbsListView = newValue;
    } // end setAbsListView

    /* end Property AbsListView */
 ...
 */
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
        ...
        // Build up a list of DropTargets from the child views of the AbsListView.
        // Tell the drag controller about them.

        if (mAbsListView != null) {
            int numVisibleChildren = mAbsListView.getChildCount();
            for (int i = 0; i < numVisibleChildren; i++) {
                DropTarget view = (DropTarget)mAbsListView.getChildAt(i);
 ...
 */
    ...
    private void scrollList(int y) {
        ...
        if (y > bound.lowerBound) {
            ...
            speed = y > (height + bound.lowerBound) / 2 ? 16 : 4;
        } else if (y < bound.upperBound) {
            // scroll the list down a bit
            speed = y < bound.upperBound / 2 ? -16 : -4;
        }
        if (speed != 0) {
            int distance = (int)(2*mDragController.getDisplayMetrics().density * speed);
            (mAbsListView).smoothScrollBy(distance, 10);
        }
    }
    ...
    private Bound getNewScrollBounds(int y) {
        int height = mAbsListView.getHeight();
        if (y >= height / 3) {
            bound.upperBound = height / 3;
        }
        if (y <= height * 2 / 3) {
            bound.lowerBound = height * 2 / 3;
        }
        return bound;
    }
